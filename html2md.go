...
func MPVoice() *Rule {
	return &Rule{
		Patterns: []string{"mpvoice"},
		Replacement: func(innerHTML string, attrs []string) string {
			var voice string
			voiceR := AttrRegExp("voice_encode_fileid")
			matches := voiceR.FindStringSubmatch(attrs[0])
			if len(matches) > 1 {
				voice = matches[1]
			}
			voiceSrc := fmt.Sprintf("https://res.wx.qq.com/voice/getvoice?mediaid=%v", voice)

			return wrapInlineTag(`音频`, "[", "]") + "(" + voiceSrc + ")"
		},
	}
}
...
func Iframe() *Rule {
	return &Rule{
		...
		Replacement: func(innerHTML string, attrs []string) string {
			var iframe string
			iframeR := AttrRegExp("data-src")
			matches := iframeR.FindStringSubmatch(attrs[0])
			if len(matches) > 1 {
				iframe = matches[1]
			}

			return wrapInlineTag(`子窗口`, "[", "]") + "(" + iframe + ")"
		},
	}
}
